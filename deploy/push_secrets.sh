...
# ---------------------------------------------------------------------------
# Set version-destroy TTL to auto-cleanup old versions (saves cost)
# Old versions are destroyed 24h after being superseded by a newer one.
# ---------------------------------------------------------------------------

echo ""
echo "=== Setting version-destroy TTL on frequently-updated secrets ==="

for secret in "$TOKEN_SECRET" "$ENV_SECRET"; do
  gcloud secrets update "$secret" \
    --version-destroy-ttl="86400s" \
    --project="$GCP_PROJECT_ID" \
    --quiet 2>/dev/null && echo "  TTL SET  $secret (24h)" || echo "  SKIP TTL $secret (may already be set)"
done
