...
def get_embeddings_model():
    """
    Select the embeddings backend.

    Defaults to Ollama for local/dev usage. If the main model endpoint is Vertex,
    embeddings follow Vertex as well unless EMBEDDING_ENDPOINT is explicitly set.
    """
    embedding_backend = os.environ.get("EMBEDDING_ENDPOINT")
    if not embedding_backend:
        embedding_backend = "vertex" if os.environ.get("MODEL_ENDPOINT") == "vertex" else "ollama"
    ...
    raise ValueError(
        f"Unknown embedding backend '{embedding_backend}'. Choose from: ollama, vertex."
    )
...
c = ChromaUtils(collection_name="rag_tennis_notes", persist_db_directory=os.environ.get("VECTOR_INDEX_DB_PATH"), embeddings_model=embeddings)
...
def get_agent(model: str = "ollama"):
    ...
    system_prompt = f"""
        ...
        * **Precipitation:** Must be less than 15% rain during the match, 6 hours BEFORE, and 4 hours AFTER.
        * **Temperature:** Must be > 40°F and < 95°F.
